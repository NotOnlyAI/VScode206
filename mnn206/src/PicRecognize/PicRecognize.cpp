...
int PicRecognize::Forward(cv::Mat &raw_image) {
    ...
    auto inputUser = new MNN::Tensor(output, MNN::Tensor::TENSORFLOW);
    ...
    pretreat->convert(image.data, in_w, in_h,0, inputUser);
    input->copyFromHostTensor(inputUser);
    ...
    for (int i = 0; i < 10; ++i) {
        MNN_PRINT("%f, %f\n", nhwcTensor->host<float>()[i], nhwcTensor->host<float>()[i]);
    }
    delete nhwcTensor;
