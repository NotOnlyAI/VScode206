...
object::object(int _id, InputArray image, const Rect2d& boundingBox, double _scale, std::string algorithm)
{
    status = 1;
...
void object::update(Mat& image, const cv::Rect2d& boundingBox)
{
    tracker.release();
    tracker = createTracker(tracker_type);
    Mat dst_img;
