...
int32_t JpegFile::InitOsdRegion(const sdc_yuv_frame_s &yuv_frame, sdc_osd_region_s &osd_region) const
{
	// 中文采用双字节编码
    wchar_t Font[] = {
    ...
    (void)memcpy(osd_region.osd.format,/*sizeof(osd_region.osd.format),*/
		"fontsize=2;fgcolor=0x111111;fgalpha=120", sizeof("fontsize=2;fgcolor=0x111111;fgalpha=120") - 1);

    (void)memcpy(osd_region.osd.content, /*sizeof(osd_region.osd.content),*/ Font, sizeof(Font));
    osd_region.osd.content_len = sizeof(Font);
	return OK;
}

int32_t JpegFile::Yuv2Jpeg(const sdc_yuv_frame_s &yuv_frame, const sdc_osd_region_s &osd_region, sdc_jpeg_frame_s &jpeg_frame) const
{
...
int32_t JpegFile::RGB2Jpeg(const sdc_yuv_frame_s &yuv_frame, const sdc_osd_region_s &osd_region, sdc_jpeg_frame_s &jpeg_frame) const
{
    sdc_encode_jpeg_param_s param;
    memset(&param,/* sizeof(param),*/ 0, sizeof(param));
    param.qf = 80;
...
int32_t JpegFile::SaveJpeg(const sdc_yuv_frame_s &yuv_frame, const string &jpegPath) const
{
...
int32_t JpegFile::SaveRGB2Jpeg(const sdc_yuv_frame_s &yuv_frame, const string &jpegPath) const
{
    sdc_osd_region_s osd_region;
    memset(&osd_region, /*sizeof(osd_region),*/ 0, sizeof(osd_region));
...
int32_t JpegFile::SaveJpeg(const sdc_jpeg_frame_s &jpeg_frame, const string &jpegPath) const
{
	int32_t fd = -1;
