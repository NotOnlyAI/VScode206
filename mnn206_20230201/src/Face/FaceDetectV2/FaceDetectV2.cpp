...
FaceDetectV2::FaceDetectV2() {

    string mnn_path="./models206/FaceDetectV2.mnn";
    ...
    output_blob_names={ "score80","score40","score20","box80","box40","box20",};
...
int FaceDetectV2::Forward(cv::Mat &raw_image) {
...
int FaceDetectV2::decode(std::vector< MNN::Tensor*> &outputTensors_host)
{
    // for (int i = 0; i < 10; ++i) {
    ...
    int grid_hs[4];
    int grid_ws[4];
    int BboxNumEachGrid[4]={2,2,2,3};
    float BoxSteps[4]={8,16,32,64};
    float BoxMinSizes[4][3] = {{8, 16}, {32,64}, {128, 256,0}, {1, 1, 1}};
    grid_hs[0]=80;grid_hs[1]=40;grid_hs[2]=20;grid_hs[3]=5;
    grid_ws[0]=80;grid_ws[1]=40;grid_ws[2]=20;grid_ws[3]=4;
    ...
    for(int i = 0; i < end; i++)
    {
        ...
        uint32_t u32Offset1=0;
        uint32_t u32Offset2=0;
        uint32_t u32Offset3=0;
        uint32_t u32OffsetScore=0;
        uint32_t u32OffsetScore0=0;
        uint32_t u32OffsetScore1=0;
        ...
        for(int h = 0; h < grid_h; h++)
        {
            for(int w = 0; w < grid_w; w++)
            {
                for(int c = 0; c < 2; c++)
                {
                    u32OffsetScore0 = w+h*grid_w+(c+0)*grid_h*grid_w;
                    u32OffsetScore1 = w+h*grid_w+(c+2)*grid_h*grid_w;
                    f32Score0=outputTensors_host[i]->host<float>()[u32OffsetScore0];
                    f32Score1=outputTensors_host[i]->host<float>()[u32OffsetScore1];
                    ...
                    if(f32Score1>0.5)
                    {
                        ...
                        // uint32_t x = j % grid_w;
                        // uint32_t y = j / grid_w;
                        float dense_cx = (w+0.5) * BoxSteps[i] / in_w;
                        float dense_cy = (h+0.5) * BoxSteps[i] / in_h;
                        float s_kx = BoxMinSizes[i][c] / in_w;
                        float s_ky = BoxMinSizes[i][c] / in_h;
                        ...
                        u32Offset0 = w+h*grid_w+(4*c+0)*grid_h*grid_w;
                        u32Offset1 = w+h*grid_w+(4*c+1)*grid_h*grid_w;
                        u32Offset2 = w+h*grid_w+(4*c+2)*grid_h*grid_w;
                        u32Offset3 = w+h*grid_w+(4*c+3)*grid_h*grid_w;

                        f32X = outputTensors_host[i+3]->host<float>()[u32Offset0]*s_kx+dense_cx;
                        f32Y = outputTensors_host[i+3]->host<float>()[u32Offset1]*s_ky+dense_cy;
                        f32Width = exp(outputTensors_host[i+3]->host<float>()[u32Offset2])*s_kx;
                        f32Height = exp(outputTensors_host[i+3]->host<float>()[u32Offset3])*s_ky;
